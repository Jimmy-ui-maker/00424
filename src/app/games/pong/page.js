"use client";
import PongGame from "@/components/GamesComponents/PongGame";
...
export default function PongPage() {
  ...
  // Load saved user
  useEffect(() => {
    const stored = localStorage.getItem("ponggame");
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setUser(form);
    localStorage.setItem("ponggame", JSON.stringify(form));
  };
    ...
    <div className="container py-5 d-flex justify-content-center">
      {!user ? (
        <div className="card col-12 col-md-6 p-4 shadow-lg position-relative">
          <h2 className="mb-4 text-center">🏓 Start Pong Game</h2>
              ...
              <div className="d-flex justify-content-center gap-3">
                {["simple", "medium", "hard"].map((level) => (
                  <div className="form-check" key={level}>
                    <input
                      className="form-check-input shadow-none disabled"
                      type="checkbox"
                      id={level}
